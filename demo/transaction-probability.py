...
#    module_file = tf.train.latest_checkpoint('/notebooks/demo/test1')
    ...
    with tf.Session() as sess:
        sess.run(init)
        save_path = saver.save(sess, "./model/stock_model")
        print("Save to path: ", save_path)

    # 提取变量
    with tf.Session() as sess:
        #sess.run(tf.global_variables_initializer())
        saver.restore(sess, './model/stock_model')
        ...
        for i in range(2000):
            for step in range(len(batch_index)-1):
                ...
                print(i,loss_)
            if i % 200 == 0:
                print("保存模型：",saver.save(sess,'./model/stock_model',global_step=i))
